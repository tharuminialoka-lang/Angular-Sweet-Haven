...
import { Category } from '../../models/sweet';
...
export class CategoriesComponent implements OnInit {
  ...
  selectCategory(categoryType: string): void {
    ...
    this.sweetService.setSelectedCategory(categoryType);
    this.categorySelected.emit(categoryType);
    this.scrollToFeaturedSection();
  }

  selectAll(): void {
    this.selectedCategory = 'all';
    this.sweetService.setSelectedCategory('all');
    this.categorySelected.emit('all');
    this.scrollToFeaturedSection();
  }

  isCategorySelected(categoryType: string): boolean {
    return this.selectedCategory === categoryType;
  }

  private scrollToFeaturedSection(): void {
    setTimeout(() => {
      const featuredSection = document.querySelector('app-featured-sweets');
      if (featuredSection) {
